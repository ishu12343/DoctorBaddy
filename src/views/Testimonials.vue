    ...
    <main class="flex-1 pt-20 lg:pt-24">
      <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12">
          <h1 class="text-4xl font-bold text-gray-900 mb-4">Patient & Doctor Stories</h1>
          <p class="text-xl text-gray-600 max-w-3xl mx-auto">Real experiences from our community of patients and healthcare professionals</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="story in visibleStories" :key="story.id" 
            ...
            <div class="p-6 flex flex-col h-full">
              ...
              <p class="text-gray-600 mb-6 italic text-lg flex-grow">"{{ story.text }}"</p>
              <div class="flex items-center gap-4 border-t border-gray-100 pt-4 mt-auto">
                <img :src="story.avatar" :alt="story.name" class="w-16 h-16 rounded-full object-cover" />
                <div>
                  <!-- Patient Details -->
                  <template v-if="story.type === 'Patient'">
                    <div class="font-semibold text-gray-900 text-lg">{{ story.name }}</div>
                    <div class="text-gray-500">{{ story.location }}</div>
                    <div v-if="story.condition" class="text-sm text-blue-600 mt-1">Treated for: {{ story.condition }}</div>
                  </template>
                  <!-- Doctor Details -->
                  <template v-if="story.type === 'Doctor'">
                    <div class="font-semibold text-gray-900 text-lg">Dr. {{ story.name }}</div>
                    <div class="text-gray-500">{{ story.specialty }}</div>
                    <div class="text-sm text-green-600 mt-1">{{ story.experience }} of experience</div>
                  </template>
                </div>
              </div>
            </div>
        ...
        <div class="mt-12 text-center flex flex-col sm:flex-row justify-center items-center gap-4">
          <button v-if="hasMoreStories" @click="loadMore" 
                  class="px-6 py-3 bg-medical-primary text-white font-medium rounded-lg hover:bg-medical-secondary transition-colors duration-300">
            Load More Stories
          </button>
          <button @click="$router.go(-1)" 
                  class="px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors duration-300">
